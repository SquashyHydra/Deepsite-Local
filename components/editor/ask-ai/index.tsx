...
export function AskAI({
  ...
  // Ensure model and provider are always valid for LM Studio
  useUpdateEffect(() => {
    // If MODELS is empty, do nothing
    if (!MODELS || MODELS.length === 0) return;
    // If current model is not in MODELS, reset to first
    if (!MODELS.find((m: any) => m.value === model)) {
      setModel(MODELS[0].value);
    }
    // If provider is not valid for selected model, reset to auto
    const selected = MODELS.find((m: any) => m.value === model);
    if (selected && provider !== "auto" && !selected.providers.includes(provider)) {
      setProvider("auto");
    }
  ...
  const handleError = (error: string, message?: string) => {
    // Prevent login popup for LM Studio (local) provider
    if (provider === "lmstudio" || (MODELS.length > 0 && MODELS[0].providers?.includes("lmstudio"))) {
      if (error === "login_required") {
        toast.error("LM Studio is running locally. No login required.");
        return;
      }
    }
    switch (error) {
  ...
  const projectdata: Project | null = project ?? null;
